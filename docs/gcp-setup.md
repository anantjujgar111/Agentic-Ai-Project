...
The deploy script grants `roles/datastore.user` to the Cloud Run runtime service account when `SESSION_BACKEND=firestore`. If `/chat` still returns 500 after deploy, redeploy once with the updated script or run:
